    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border border-border/50" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="font-serif font-bold text-foreground text-lg">Claim This Shop</h3>
            <p className="text-xs text-muted-foreground">{shop.name}</p>
          </div>
        </div>
        {!submitted ? (
          <form name="claim-shop" method="POST" data-netlify="true" onSubmit={handleSubmit} className="space-y-3">
            <input type="hidden" name="form-name" value="claim-shop" />
            <input type="hidden" name="shopName" value={shop.name} />
            <input type="hidden" name="shopAddress" value={shop.address} />
            <input required placeholder="Your name *" value={form.ownerName}
            ...
            <input required type="email" placeholder="Email address *" value={form.email}
              ...
              className="w-full px-3 py-2.5 text-sm border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30" />
            <input placeholder="Phone number (optional)" value={form.phone}
              onChange={e => setForm(p => ({ ...p, phone: e.target.value }))}
              className="w-full px-3 py-2.5 text-sm border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30" />
            <textarea placeholder="Tell us about your shop or any corrections needed..." value={form.message}
              onChange={e => setForm(p => ({ ...p, message: e.target.value }))}
              rows={3} className="w-full px-3 py-2.5 text-sm border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none" />
            <p className="text-xs text-muted-foreground">We'll verify your ownership and reach out within 2-3 business days.</p>
            <button type="submit" className="w-full py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:bg-primary/90 transition-colors">
              Submit Claim
            </button>
          </form>
        ) : (
          <div className="text-center py-8">
            ...
            <p className="font-serif font-bold text-foreground mb-1">Claim Submitted!</p>
            <p className="text-sm text-muted-foreground">We'll review your claim and reach out to your email soon.</p>
          </div>
        )}
      </div>
    </div>
        ...
        <div className="overflow-y-auto max-h-[90vh]">
          ...
          <div className="p-8">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {openNow !== null && (
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${openNow ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
                  ...
                  {weekdayDescriptions.length > 0 ? (
                    <>
                      <button onClick={() => setShowHours(h => !h)} className="text-xs text-primary hover:underline mt-0.5">
                    ...
                    <a href={`https://www.google.com/search?q=${encodeURIComponent(shop.name + " " + shop.address + " hours")}`}
                      target="_blank" rel="noopener noreferrer"
                      className="text-xs text-muted-foreground hover:text-primary hover:underline mt-0.5 block">
            ...
            <button onClick={() => setShowClaim(true)}
              className="w-full mt-3 flex items-center justify-center gap-2 py-2 text-xs text-muted-foreground hover:text-primary transition-colors">
              <Flag className="w-3 h-3" /> Own this shop? Claim your listing
            </button>
            <ReviewSection shopId={shop.id} />
          </div>
        </div>
      </div>
      {showClaim && <ClaimModal shop={shop} onClose={() => setShowClaim(false)} />}
    </div>
  );
}
...
export function CoffeeCard({ shop, forceOpen, onModalClose }: { shop: CoffeeShop; forceOpen?: boolean; onModalClose?: () => void }) {
  ...
  // Close when home navigation is triggered
  useEffect(() => {
    const onCloseAll = () => { setOpen(false); onModalClose?.(); };
